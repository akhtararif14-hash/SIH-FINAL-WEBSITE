...
export function buildPrompt(report, lang = 'en') {
  ...
  const facts = {
    ...
    userBudgetRupees: report.budget || 'not given',
        // Real customer reviews of nearby shops (only when Google reviews are on).
    whatCustomersSayNearby: (report.reviewNotes || []).slice(0, 12),
    topBusinesses: top.map((r) => ({
      ...
      specialNote: r.note,
            moneyEstimate: r.finance
        ? {
            totalInvestmentRupees: r.finance.totalInvestment,
            monthlySalesRupees: r.finance.monthlyRevenue,
            monthlyProfitRupees: r.finance.netProfit,
            paybackMonths: r.finance.paybackMonths,
          }
        : null,
    })),
  };
...
3. One line: the money picture — use the moneyEstimate numbers (investment, monthly profit, payback months) and call them estimates.
4. One line: a practical first step.
If whatCustomersSayNearby is not empty, add a short section titled "What customers complain about nearby" with 2-3 bullet points naming the common complaints (quote nothing word-for-word, just summarise) and how the new shop can do better.
...
Keep the whole answer under 260 words. Plain text, no markdown tables.
